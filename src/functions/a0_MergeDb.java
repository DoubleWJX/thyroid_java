...
public class a0_MergeDb {
	...
	private final static String dirCommon = "D:/甲状腺标注/甲状腺标注数据20170525/"; // 这个是存放标注数据的本地路径
	...
	public static void insertIntoDb(String fromPath, String toPath, int index){
	    ...
	    try {
	      ...
	      while ( rs.next() ) {
	    	  ...
	    	  path = dirCommon + personalDir + path.replace(dirCommonOld, "").replace(dirCommonOld2, "").replace(dirCommonOld3, "");
